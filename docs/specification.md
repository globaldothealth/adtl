...
flag (or the equivalent `include_transform` argument to `adtl.parse()`)
and providing a single python file where their custom functions are provided. Custom functions will only execute if using Python's stdlib, or one of adtl's dependencies. Parameters other than the source field which need to be parsed into the transformation
